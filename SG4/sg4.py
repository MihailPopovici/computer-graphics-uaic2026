...
class OglSimpleRenderer():
    ...
    def integrate(self, obj, accel = [0, 0, 0], dt = 1, collisionElasticity = 0.99, collisionFriction = 0.002):
        ...
        if dt <= 0: #daca delta t e 0 sau negativ, nu facem nimic, ca nu are sens sa integram cu un timp negativ sau 0, deci iesim din functie.
            return

        #As a general rule
        #i=0 -> axa X; i=1 -> axa Y; i=2 -> axa Z

        # Semi-implicit Euler: integrate speed first, then position.
        drag_coeff = 0.4 #The higher, the more drag. 0 means no drag, 1 means stop immediately.
        drag = max(0.0, 1.0 - drag_coeff * dt)
        for ii in range(3):
            vel[ii] += accel[ii] * dt #viteza e acc * timp
            vel[ii] *= drag     #ii punem drag
            pos[ii] += vel[ii] * dt #pozitia e viteza * timp

        # Keep particles inside the scene volume and bounce on collision.
        frictionScale = max(0.0, 1.0 - collisionFriction)
        floor = self.sceneVolume[4]
        for ii in range(3):
            min_boundary = self.sceneVolume[2 * ii]       # generally, sceneVolume[i] is a limit of the scene (either min or max)
            max_boundary = self.sceneVolume[2 * ii + 1]   # values of i: (0,1) are for x, (2,3) for y, (4,5) for z axis
            #center = pos[ii] + c[ii]
            collided = False

            if pos[ii] < min_boundary:  #daca pozitia e mai mica decat limita minima
                pos[ii] = min_boundary  #dam snap pozitiei la limita minima 
                #pos[ii] = min - c[ii]
                collided = True
            elif pos[ii] > max_boundary:    #daca pozitia e mai mare decat limita maxima
                pos[ii] = max_boundary      #dam snap pozitiei la limita maxima
                #pos[ii] = max - c[ii]
                collided = True

            if collided:
                vel[ii] = -vel[ii] * collisionElasticity    #inversam viteza pe axa respectiva, si o dam cu elasticitatea
                for jj in range(3): 
                    if jj != ii:                #pentru celelalte axe, aplicam frictiunea de coliziune, care reduce viteza in acele directii
                        vel[jj] *= frictionScale #frictionScale e 1 minus collisionFriction, deci daca collisionFriction e 0.002, atunci frictionScale e 0.998, adica pierdem 0.2% din viteza in celelalte directii la fiecare coliziune.

        
        # Floor contact mode: kill tiny vertical jitter but keep horizontal sliding.
        #Nu prea merge feature-ul asta:( 
        floorContact = abs((pos[2] + c[2]) - floor) < 0.03
        if floorContact and abs(vel[2]) < 1.2:
            vel[2] = 0.0
            slideDamping = max(0.0, 1.0 - 0.2 * dt)
            vel[0] *= slideDamping
            vel[1] *= slideDamping
            if abs(vel[0]) < 0.02:
                vel[0] = 0.0
